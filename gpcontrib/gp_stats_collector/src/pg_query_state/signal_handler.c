 ...
 * (T_SeqScan 27 -> 395).  Sending nodeTag(plan) raw therefore made every node
 * unresolvable on a receiver holding a PG14-era table.  Map explicitly so the
 * protocol is decoupled from the backend's internal numbering.
 *
 * The cases below mirror the node-name switch in ExplainNode(); anything not
 * listed is reported as UNSPECIFIED rather than guessed.
 */
...
qs_map_node_type(NodeTag tag)
{
	switch (tag)
	{
		...
		case T_SplitMerge:				return QS_PLAN_NODE_TYPE_SPLIT_MERGE;
